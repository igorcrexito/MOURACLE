...
#loading test images ---------------------------------
base_image_path = 'D:/Sample0463/'
number_of_frames = len(fnmatch.filter(os.listdir(base_image_path), '*.jpg'))
print('number of frames: '+ str(number_of_frames))

#iterating over frames
for fi in range(0, number_of_frames):
	test_image = base_image_path + str(fi+1) + '.jpg'
	print(test_image)
    
	#test_image = 'C:/Users/igorc/Downloads/Train/Sample0001/1.jpg'
	...
	base_writing_path = base_image_path
	with open(base_image_path+"/joints.txt", "a") as myfile:
		for zindex in range(0, len(all_peaks)):
			peak = all_peaks[zindex]
			if (len(peak)>0):
				myfile.write(str(peak[0][0]) + ' ')
				myfile.write(str(peak[0][1]) + ' ')
				myfile.write(str(peak[0][3]) + ' ')
				myfile.write('; ')
		myfile.write('\n')
